...
export default function LeaderboardPage() {
  ...
  return (
    <div className="container mx-auto px-4 max-w-5xl py-8 flex-grow flex flex-col justify-start">
      {/* Header Back & Info */}
      <div className="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-5 select-none">
        <div>
          <Link 
            href="/exam"
          ...
          <p className="text-zinc-500 text-xs mt-1.5 leading-relaxed">
            Xem thành tích xuất sắc nhất của các học viên. Thử sức làm bài thi để ghi danh trên bảng vàng nhé!
          </p>
        </div>

        {/* Dynamic Status Display */}
        {!loadingRankings && (
          <div className="bg-gradient-to-br from-orange-50 to-amber-50/50 border border-orange-100 rounded-2xl px-5 py-3.5 dark:from-orange-950/15 dark:to-zinc-900 dark:border-orange-900/30 flex items-center gap-3.5 shadow-sm">
      ...
      <Card className="border border-zinc-200/80 bg-white/80 dark:bg-zinc-900/80 dark:border-zinc-800/80 backdrop-blur-md rounded-3xl shadow-sm overflow-hidden flex-1 flex flex-col">
                ...
                <tbody className="divide-y divide-zinc-100/80 dark:divide-zinc-800/40">
                  {filteredRankings.map((entry, idx) => {
                    ...
                    if (rank === 1) {
                      ...
                      rankDisplay = (
                        <span className="font-bold text-zinc-500 text-xs block text-center">{rank}</span>
                      );
                    }
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div className="flex flex-col truncate">
                              <span className="font-bold text-xs text-zinc-800 dark:text-zinc-200 flex items-center gap-1">
                                ...
                                {entry.userEmail && (
                                  <Badge 
                                    className="bg-blue-50 text-blue-600 border-none rounded px-1.5 py-0 h-4 dark:bg-blue-950/30 dark:text-blue-400 font-semibold text-[8px] flex items-center gap-0.5 select-none"
                                  >
